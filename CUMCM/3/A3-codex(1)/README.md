...
> 当前组合衔接说明（优先于下文原联合运行说明）：本版本与用户选定的04_and_04pic采用两套独立实现，并未共用生产内核。固定半径主结果保持57.169383 h，严格整分钟保持205860 s；收缩主结果保持50.8242826 h，严格整分钟保持183000 s。两者统一采用显热主模型，潜热仅为收缩工况扩展。当前副本缺少verification目录，以下验证指标为原报告记录，不是本次重新验收结果。原联合入口依赖旧A3/A4目录布局，会联动生成另一收缩版本；补齐验证材料及确认输出路径前，不应执行下列联合复算/发布命令。原说明保留供溯源，不代表当前组合可直接一键复现。
