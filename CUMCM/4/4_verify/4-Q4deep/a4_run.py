...
def chamber_tail(scenario):
    ...
    if scenario == 'final':
        ...
        w = np.ones(11) / 11.0
        tail = [np.convolve(T, w, mode='same')[-1], np.convolve(C, w, mode='same')[-1]]
    else:
...
class Q4Solver:
    def __init__(self, n=N_DEFAULT, rtol=RTOL, scenario='final', mesh='graded',
        ...
        if scenario == 'smooth11':
            w = np.ones(11) / 11.0
            self.t_ch = M.CH_T
            self.T_ch = np.convolve(M.CH_TC, w, mode='same')
            self.C_ch = np.convolve(M.CH_CC, w, mode='same')
        ...
        self.rhoc0 = {'dry0': self.rho_d0, 'dry': None, 'bulk': None, 'unit': 1.0}[rmode]
    ...
    def _rhoc0_from_state(self, y):
        if self.rhoc0 is not None:
            return self.rhoc0
        Cc = np.clip(y[1::2], 1e-12, None)
        if self.rhoc_mode == 'dry':
            rho = (760. + 90. * Cc) if self.appx == 4 else (650. + 128. * Cc)
            return float(np.mean(rho / (1. + Cc)))
        rho = (760. + 90. * Cc) if self.appx == 4 else (650. + 128. * Cc)
    ...
    def integrate(self):
        ...
        while left < self.tmax - 1e-6:
            right = min(left + 60.0, 14400.0) if left < 14400.0 else left + 24.0 * 3600.0
            ...
            end = right if crossing is None else min(right, (np.floor(crossing / 60.0) + 1) * 60.0)
            ts = np.arange(left + self.sample_dt, end + 1e-6, self.sample_dt)
            if len(ts):
                ys = sol.sol(ts)
                times.extend(ts)
                states.extend(ys.T)
...
def solve_case(name, n=N_DEFAULT, rtol=RTOL, scenario='final', mesh='graded',
               ...
               dist_cols=None, save=True, return_solver=False):
    ...
    fld = M.sample_many(states.T, s.x, s.half, airs, R_arr, qs, M.HCONV, M.HM, s.df, appx)
    ...
    # table 6 rows: 6,12,...,48 h + end; columns 0 / 0.5 / 1.0 cm + surface
    t6_rows = [th for th in [6., 12., 18., 24., 30., 36., 42., 48.] if th * 3600.0 < minute]
    ...
    fld6 = M.sample_many(st6, s.x, s.half, airs6, Rs6, qs6, M.HCONV, M.HM, s.df, appx)
    res = dict(name=name, n=n, rtol=rtol, scenario=scenario, mesh=mesh,
               latent=latent, appx=appx, crossing_s=crossing, minute_s=minute,
...
if __name__ == '__main__':
    ...
    p.add_argument('--latent-rho', default='dry0', choices=['dry0', 'dry', 'bulk', 'unit'])
    ...
    solve_case(a.name, n=a.n, rtol=a.rtol, scenario=a.scenario, mesh=a.mesh,
               ...
               appx=a.appx, sample_dt=a.sample_dt)
