...
事件函数是所有单元、对称重构中心和真实表面的最大含水率减0.15。中心按r²重构，内部目标点三点二次插值，表面用边界解。每个60 s输出检查全部单元的径向顺序，本次最大向外增量为8.882e-16（舍入误差量级），支持中心最湿判断；这不是连续空间的严格误差界。
...
|---|---:|---:|---:|
| 400_to_800 | 7.074e-06 | 1.388e-05 | 0.953534 |
| 800_to_1600 | 1.769e-06 | 3.474e-06 | 0.238539 |
| tolerance | 1.042e-09 | 1.404e-08 | 0.000017 |
| BDF_vs_Radau | 1.151e-09 | 1.601e-08 | 0.000030 |
...
对照第二问已保存的tight800轨迹，0—10800 s每60 s采样最大差为：C 5.775e-12 kg/kg、T 6.641e-10℃，说明第三问准确承接第二问。独立向量算子在最终状态与非均匀合成状态上复核物性、面通量、几何及独立非线性边界根，详情见verification/summary.json。

水分积分量M=ΣV_iC_i（不直接等于kg），恒等式Ṁ=−Q_C(R)。独立三点Gauss时间积分边界通量后，相对闭合误差为2.668e-11。

显热诊断量E=ΣV_iA(C_i)T_i，链式法则给出

$$\dot E=-Q_T(R)+\sum_i V_iA'(C_i)T_i\dot C_i.$$

其中第二项是变热容的导数修正，不是往PDE加入焓输运。积分闭合相对误差为3.011e-10；质量和热方程的离散恒等式绝对残差分别为6.617e-24、1.388e-17。这些验证只说明所选有效PDE和显热方程被正确离散，**不代表完整蒸发质量—能量模型守恒**。未通过调整通量样本消除残差。
